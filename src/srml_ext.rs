...
impl<T> SrmlExt for AuraCall<T>
where
    T: srml_aura::Trait,
{
    fn function(&self) -> SrmlResult<FunctionInfo> {
        match &self {
            &__phantom_item => Ok(("".into(), Vec::new())),
        }
    }
}

impl<T> SrmlExt for BabeCall<T>
where
    T: srml_babe::Trait,
{
    fn function(&self) -> SrmlResult<FunctionInfo> {
...
impl<T> SrmlExt for SessionCall<T>
where
    T: srml_session::Trait,
{
    fn function(&self) -> SrmlResult<FunctionInfo> {
        match &self {
            SessionCall::set_keys(keys, proof) => Ok((
                "set_keys".into(),
                vec![keys.encode(), proof.encode()].encode(),
            )),
            &__phantom_item => Ok(("".into(), Vec::new())),
        }
    }
...
impl<T> SrmlExt for TimestampCall<T>
where
    T: srml_timestamp::Trait,
{
    fn function(&self) -> SrmlResult<FunctionInfo> {
        match &self {
            TimestampCall::set(time) => Ok(("set".into(), vec![time.encode()].encode())),
            &__phantom_item => Ok(("".into(), Vec::new())),
        }
    }
}

impl<T> SrmlExt for FinalityCall<T>
where
    T: srml_finality_tracker::Trait,
{
    fn function(&self) -> SrmlResult<FunctionInfo> {
        match &self {
...
impl<T> SrmlExt for ImOnlineCall<T>
where
    T: srml_im_online::Trait,
{
    fn function(&self) -> SrmlResult<FunctionInfo> {
        match &self {
            ImOnlineCall::heartbeat(heartbeat, signature) => Ok((
                "im-online".into(),
                vec![heartbeat.encode(), signature.encode()].encode(),
            )),
            &__phantom_item => Ok(("".into(), Vec::new())),
        }
    }
...
impl<T> SrmlExt for StakingCall<T>
where
    T: srml_staking::Trait,
{
    fn function(&self) -> SrmlResult<FunctionInfo> {
        match &self {
            StakingCall::bond(controller, value, payee) => Ok((
                "bond".into(),
                vec![controller.encode(), value.encode(), payee.encode()].encode(),
            )),
...
impl<T> SrmlExt for GrandpaCall<T>
where
    T: srml_grandpa::Trait,
{
    fn function(&self) -> SrmlResult<FunctionInfo> {
        match &self {
...
impl<T> SrmlExt for SudoCall<T>
where
    T: srml_sudo::Trait,
{
    fn function(&self) -> SrmlResult<FunctionInfo> {
        match &self {
            SudoCall::sudo(proposal) => Ok(("sudo".into(), vec![proposal.encode()].encode())),
            SudoCall::set_key(source) => Ok(("set_key".into(), vec![source.encode()].encode())),
            &__phantom_item => Ok(("".into(), Vec::new())),
        }
    }
}

impl<T> SrmlExt for TreasuryCall<T>
where
    T: srml_treasury::Trait,
{
    fn function(&self) -> SrmlResult<FunctionInfo> {
        match &self {
            TreasuryCall::propose_spend(value, beneficiary) => Ok((
                "propose_spend".into(),
                vec![value.encode(), beneficiary.encode()].encode(),
            )),
            TreasuryCall::reject_proposal(proposal_id) => Ok((
                "reject_proposal".into(),
                vec![proposal_id.encode()].encode(),
            )),
            TreasuryCall::approve_proposal(proposal_id) => Ok((
                "approve_proposal".into(),
                vec![proposal_id.encode()].encode(),
            )),
            &__phantom_item => Ok(("".into(), Vec::new())),
        }
