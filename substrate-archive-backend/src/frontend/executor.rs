...
use sp_state_machine::{self, ExecutionManager, ExecutionStrategy, Ext, OverlayedChanges, StateMachine, StorageProof};
...
impl SpawnNamed for SpawnWrapper {
	fn spawn(&self, n: &'static str, fut: std::pin::Pin<Box<dyn Future<Output = ()> + Send + 'static>>) {
		self.0.spawn(n, fut)
	}

	fn spawn_blocking(&self, n: &'static str, fut: std::pin::Pin<Box<dyn Future<Output = ()> + Send + 'static>>) {
...
impl<B, E> ArchiveExecutor<B, E> {
	...
	pub fn new(backend: Arc<B>, executor: E, spawn_handle: impl SpawnNamed + Send + Sync + 'static) -> Self {
		let spawn_handle = SpawnWrapper(Arc::new(spawn_handle));
		ArchiveExecutor { backend, executor, spawn_handle }
	}
}
	...
	Block: BlockT,
{
	...
	fn call(
		...
		.execute_using_consensus_failure_handler::<_, NeverNativeValue, fn() -> _>(strategy.get_manager(), None)?;
	...
	fn runtime_version(&self, id: &BlockId<Block>) -> sp_blockchain::Result<RuntimeVersion> {
		...
		let mut ext = Ext::new(&mut overlay, &mut offchain_overlay, &mut cache, &state, None, None);
		let state_runtime_code = sp_state_machine::backend::BackendRuntimeCode::new(&state);
		self.executor
