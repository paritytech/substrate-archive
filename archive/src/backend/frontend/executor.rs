...
#[derive(Clone)]
struct SpawnWrapper(Arc<dyn SpawnNamed>);

impl SpawnNamed for SpawnWrapper {
    fn spawn(&self, s: &'static str, future: futures::future::BoxFuture<'static, ()>) {
        self.0.spawn(s, future)
    }

    fn spawn_blocking(&self, s: &'static str, future: futures::future::BoxFuture<'static, ()>) {
        self.0.spawn_blocking(s, future) 
    }
}
...
impl<B, E> ArchiveExecutor<B, E> {
    ...
    pub fn new(backend: Arc<B>, executor: E, spawn_handle: impl SpawnNamed + 'static) -> Self {
        ArchiveExecutor {
            backend,
            executor,
